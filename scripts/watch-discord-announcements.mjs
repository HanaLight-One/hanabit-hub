...
async function catchUp(reason) {
  if (!channel || !collector) return;
  if (catchupInFlight) return catchupInFlight;
  catchupInFlight = (async () => {
    const summary = await collector.collectRecent(channel, { limit: 100 });
    await safeLog(
      `Discord 공지 보충 확인(${reason}): 조회 ${summary.scanned}, ` +
...
try {
  ...
  collector = createDiscordAnnouncementCollector({
    stateRoot,
  ...
  client.on("messageCreate", async (message) => {
    if (message.channelId !== config.openaiChannelId) return;
    try {
      const result = await collector.collectMessage(message);
      if (result.status === "created") {
        await safeLog(`Discord 새 공지 즉시 수집: 이미지 ${result.mediaCount}`);
      }
    } catch (error) {
  ...
  channel = await guild.channels.fetch(config.openaiChannelId);
  if (
    !channel ||
  ...
  await catchUp("시작");
  await safeLog("Discord 공지 실시간 감시 시작");
  setInterval(() => {
