...
import { loadXStreamPolicy } from "../src/modules/news/x-watch-source.mjs";

const PROJECT_ROOT = fileURLToPath(new URL("../", import.meta.url));
dotenv.config({ path: path.join(PROJECT_ROOT, ".env"), quiet: true });
const policy = await loadXStreamPolicy(path.join(PROJECT_ROOT, "config", "news-x-sources.json"));
const rule = buildXStreamRule(policy);
...
if (!apply) {
  console.log(JSON.stringify({
    ...
    lanes: policy.groups.map(({ id, handles }) => ({ id, handles })),
    rule: rule.value,
  }, null, 2));
  process.exit(0);
}
