...
`src/modules/news/openai-status-monitor.mjs`의 상태 영수증으로 공식 업데이트 ID, 활성 장애,
장애 상태인 개별 서비스 구성요소를 함께 비교한다. 사건 등록 전 구성요소의 노란불·빨간불도
보조 장애 신호로 사용하되 계획 점검은 제외한다. 상태 CDN의 낡은 응답을 반복 사용하지 않도록
매 요청에 시간 기반 캐시 키와 `no-store`를 적용한다. 첫 실행과 v1 영수증 이식 시 현재 상태를
기준선으로만 저장해 과거 속보를 재게시하지 않는다.
새 장애·확대·현황 변경·부분 복구는 직전 자동 상태 글의 삭제가 `deleted`로 확정된 뒤에만
새 파딱 글을 게시한다. 삭제 결과가 불명확하면 새 게시를 보류해 상태 글이 누적되지 않게 한다.
...
상태 뉴스 ID는 현재 스냅샷뿐 아니라 직전 스냅샷과 전환 단계까지 포함해, 서로 다른 장애 주기의
동일한 정상 상태가 과거 복구완료 원고를 재사용하지 않게 한다.
...
X 원문의 `t.co` 주소는 수집 단계에서 안전한 HTTPS 목적지로 한 번 풀어 실제 외부 링크를
...
원문 번역 제목과 실제 게시 제목은 분리한다. 게시 제목은 원문과 직접 연결 문맥을 함께
근거로 50자 이내의 완결된 문장으로 만들며, 길이를 맞추려고 중간에서 자르지 않는다.
정보성 제목을 만들지 못하면 자동 게시 준비를 차단한다. 내부 판정문인 `triage.reason`은
DC 원고에 싣지 않고 독자용 요약·원문 번역·관련 글 번역만 공개한다.
인물 영입·팀 합류 공지는 `src/modules/news/news-person-context.mjs`의 검증된 공개 이력을
분석 문맥으로 붙여 누구인지와 관련 경력을 독자용 요약에 설명한다. 등록된 이력이 없거나
새 팀에서의 구체적인 역할이 확인되지 않은 경우에는 내용을 지어내지 않고 자동 게시를
사람 검토로 보낸다. 공개 이력은 DC 원고에서 `인물 소개` 문단으로 관련 글과 구분한다.
같은 사건 병합은 모든 태그에 먼저 적용하되, 일반 공식 속보와 중요도 높은 `[확정]`은
속보성을 위해 15분 연속 게시 제한을 면제한다. 같은 수집 회차의 공식 변경 기록은 가장 강한
한 건만 자동 게시하고 나머지는 허브에 대기시킨다. `[사례]`는 확인된 출처·높은 중요도·신뢰도
85% 이상을 기본 자동 후보로 삼고, 확인된 신뢰 인물이 직접 연결한 공식 사례는 중간 중요도도
후보에 포함한다. 게시 사이에는 4시간을 둔다. 같은 시각에는 정보성 `[유추]`가
`[사례]`보다 우선하며, 일반 활용 사례는 허브에만 보관한다.
`src/modules/news/news-translation-audit.mjs`는 무료 API가 만든 원문 번역과 관련 글 번역의
경계·링크·영문명·수치를 로컬에서 재검사한다. 본문 자체의 한국어 존재와 영문 우세 여부를
확인하고, `openai-docs`에는 공식 출처명 `OpenAI`를 허용한다. 새 뉴스는 `news-processor.mjs`가 감사 영수증을
저장하고 기존 뉴스는 `news-reader.mjs`가 읽기 전용으로 같은 검사를 적용한다.
자동 게시 서비스는 현재 `NEWS_ANALYSIS_POLICY_VERSION`으로 실제 처리된 항목만 받는다.
정책 버전이 올라가도 이전 대기함 항목을 재기동 시점에 소급 게시하지 않는다.
DC 원고는 `src/modules/news/news-dc-copy.mjs`가 태그·출처·번역·AI 해설·원문 링크를
결정적으로 조립한다. 그림 이모지와 결합문자를 게시 전에 제거 또는 차단하고, 알려진
`/sk` 위험 경로 링크는 원고에서 제외한다. 공식 Fast 가격표는 Markdown 표 대신
DC 모바일에서 읽기 쉬운 모델별 짧은 문맥·긴 문맥 문단으로 조립한다. 실제 게시 진입점은
...
뉴스 게시자는 외부 게시자 `.env`의 파딱 전용 계정 변수를 사용하며, Hub DC 편집기의
기존 하나빛 계정 변수와 분리한다.
...
실행한다. 게시자는 `chatgpt` 갤러리, 허용된 말머리, 최대 50장, 이미지 해시,
그림 이모지·결합문자 부재와 워터마크 필드 제외를 다시 검증한다. 성공 영수증 또는
모호함 영수증이 생기면 같은 초안을 자동 재제출하지 않는다.

DC 편집기의 실제 게시자는 외부 게시자 `.env`의 기존 `DC_ID`, `DC_PW`를 사용한다.
오늘의테마·오늘의운세·뉴스 자동 게시용 파딱 계정과 섞지 않는다.

본문은 최대 101개의 텍스트·이미지 블록으로 구성해 이미지 50장 사이에도 텍스트를
배치할 수 있다. 게시 작업에는 이미지마다
...
- 이미지별 `그대로 재생성` -> `POST /api/images/generation-jobs/:id/regenerate`가 완료·실패
  작업의 해당 슬롯에서 프롬프트·인물·화풍만 새 1장 초안으로 복원한다. 기존 결과나 소스 이미지는
  새 레퍼런스로 전달하지 않는다.
- 추가 생성 자동 선택은 초안 ID로 결정적으로 인물 관계 그룹과 저장 화풍을 고르고,
  worker 시작 전에 실제 인물·화풍 ID를 작업 JSON과 이미지 제작 기록에 확정한다.
- 생성실 인물 목록은 자산 색인의 `source`와 `guest_scope`를 사용해 `에테르 대예배당`,
  `같은 세계관 특별 게스트`, `세계관 밖 특별 게스트`로 구분한다. 특별 게스트는 공용 TXT의
  압축 앵커를 혼합 cast에 사용하고, 이름과 같은 외부 폴더에 전용 텍스트·이미지 앵커가 있으면
  단독 생성과 이미지 앵커 선택에 사용한다.
- `세계관 밖 특별 게스트`는 허브 수동 생성 전용이며 기존 오테 특별 게스트 자동 추첨에는
  들어가지 않는다. `guest_scope`가 없는 이전 색인은 기존과 같이 같은 세계관 게스트로 취급한다.
- 이미지 카드의 같은 조합·인물 유지·화풍 유지 링크는 옵션 목록을 먼저 만든 뒤 제작 기록의
  프롬프트와 해당 인물·화풍 ID만 폼에 복원하며, 이전 결과 이미지는 worker 레퍼런스로
  전달하지 않는다. 별도 `이미지 재사용` 링크나 소스 선택창으로 고른 이미지만
  `sourceImageId`와 `user_reference_image`로 전달한다.
