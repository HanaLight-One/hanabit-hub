...
X 영상 게시물은 공식 API의 내부 미디어 메타데이터만 대기함에 보관한다. 주 게시물에 영상이
없으면 연결된 관련 글을 순서대로 확인해 첫 영상 메타데이터를 사용한다. DC 게시 직전에
...
삭제한다. X 원문 링크는 게시 직전 DC OGP API 응답을 검증해 전용 카드 저장 블록으로 만들며,
...
속보성을 위해 15분 연속 게시 제한을 면제한다. `[사례]`·`[유추]` 등은 기존 제한을 유지한다.
