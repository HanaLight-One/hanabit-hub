...
const OPENAI_LEARN_HOST = "learn.chatgpt.com";
...
function canonicalOpenAiKnowledgeDocUrl(value) {
  try {
    const url = new URL(String(value ?? ""));
    if (url.protocol !== "https:" || url.hostname.toLowerCase() !== OPENAI_LEARN_HOST) return null;
    if (url.port || !url.pathname.startsWith("/docs/")) return null;
    url.hostname = OPENAI_LEARN_HOST;
    url.search = "";
    url.hash = "";
    url.pathname = url.pathname.replace(/\/+$/u, "");
    return url.href;
...
function readerUrl(target) {
  ...
  url.pathname = `/https://${source.hostname}${source.pathname}`;
  return url.href;
}

function cleanReaderMarkdown(value, target, canonicalize = canonicalOpenAiArticleUrl) {
  const text = String(value ?? "").replace(/\r\n?/gu, "\n");
  const source = text.match(/^URL Source:\s*(https:\/\/\S+)\s*$/mu)?.[1];
  if (canonicalize(source) !== target) return null;
...
export function findOfficialOpenAiKnowledgeDoc(record) {
  const links = Array.isArray(record?.original?.links) ? record.original.links : [];
  return links.map(canonicalOpenAiKnowledgeDocUrl).find(Boolean) ?? null;
}

export function findOfficialOpenAiDocument(record) {
  return findOfficialOpenAiArticle(record) ?? findOfficialOpenAiKnowledgeDoc(record);
}
...
async function enrichOfficialKnowledgeDoc(record, { fetchImpl, timeoutMs }) {
  const target = findOfficialOpenAiKnowledgeDoc(record);
  if (!target) return record;
  const contexts = Array.isArray(record?.original?.contexts) ? record.original.contexts : [];
  if (contexts.some((context) => context?.relation === "official-document" &&
    canonicalOpenAiKnowledgeDocUrl(context?.url) === target)) {
    return record;
  }
  try {
    const response = await fetchImpl(readerUrl(target), {
      headers: { accept: "text/markdown, text/plain;q=0.9", "user-agent": "HanabitNewsLab/0.1" },
      signal: AbortSignal.timeout(timeoutMs),
    });
    if (!response.ok) return record;
    const contentType = String(response.headers?.get?.("content-type") ?? "").toLowerCase();
    if (contentType && !contentType.includes("text/plain") && !contentType.includes("text/markdown")) return record;
    const declaredLength = Number(response.headers?.get?.("content-length")) || 0;
    if (declaredLength > MAX_DOCUMENT_BYTES) return record;
    const raw = await response.text();
    if (Buffer.byteLength(raw, "utf8") > MAX_DOCUMENT_BYTES) return record;
    const content = cleanReaderMarkdown(raw, target, canonicalOpenAiKnowledgeDocUrl);
    if (!content) return record;
    return {
      ...record,
      original: {
...
export async function enrichOfficialDocument(
  ...
  const knowledge = await enrichOfficialKnowledgeDoc(article, { fetchImpl, timeoutMs });
  return enrichOfficialPricing(knowledge, { fetchImpl, timeoutMs });
}

export const officialDocumentEnrichmentPolicy = Object.freeze({
  hosts: Object.freeze([...OPENAI_HOSTS]),
  knowledgeHost: OPENAI_LEARN_HOST,
  knowledgePathPrefix: "/docs/",
