...
응답은 제공하지 않는다. 생성실에서는 자동 선택 옆에 `프롬프트 화풍 사용`과
Hyper-realistic, Hyper-realistic-anime, Semi-realistic-anime,
2.5D Semi-realistic-anime reality-forward 고정 렌더링을 제공한다. 저장된 긴 화풍
목록은 기본적으로 접어 현재 선택만 요약한다.
...
추가생성 입력은 최대 12,000자까지 허용한다. `새 장면`에서 등장인물 없음과
프롬프트 화풍 또는 고정 렌더링을 고르면 `prompt-only` 초안으로 분류하여 운영
화풍 자산 매칭을 요구하지 않는다. 초안 저장 자체는 Python, 무료 텍스트 API와
이미지 worker를 실행하지 않으며 두 번째 명시적 확인이 있어야 1장 실행한다.
...
핑크브릿지만 선택한 안내 생성은 자산 색인의 전용 외형 앵커와 사용자 장면을
결합하고, 프롬프트 화풍 또는 네 가지 고정 렌더링을 worker의 `locked style`로
전달해 1장 Responses worker 실행을 허용한다. 프롬프트 화풍은 natural 기본
실사·반실사 지시를 추가하지 않는다. 다른 일반 인물, 복수 인물, 운영 화풍 자산
결합과 원본 유지 모드는 계속 거부한다. 생성실은 긴 프롬프트를
