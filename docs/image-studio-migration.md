...
2. 기존 Image Studio의 API 동작을 회귀 테스트로 기록한다.
3. 파일 접근 코드를 설정 기반 어댑터로 옮긴다.
4. 기존 화면을 `/images` 아래에서 새 API와 연결한다.
5. 8790에서 읽기 전용 기능부터 병행 검증한다.
6. 생성, 휴지통, 복원 기능을 차례로 검증한다.
7. 사용자 승인 후에만 8787 시작 경로를 새 저장소로 전환한다.
