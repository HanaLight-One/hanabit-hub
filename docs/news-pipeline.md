...
원문이 이모지·기호뿐이거나 정보성 제목을 만들기에는 지나치게 짧으면 제목에 한해 직접
연결된 첫 관련 글의 번역을 근거로 짧은 한국어 요약을 만든다. 본문 번역에는 이 예외를
적용하지 않으며 항상 원문만 번역한다. DC 원고 조립도 기존 기록의 제목이 이모지 제거 뒤
비면 같은 관련 글 번역의 첫 문장으로 제목을 복구한다.
원문·관련 글 작성자 이름이 들어가는 본문 소제목도 같은 이모지 정리를 거친 뒤 게시한다.
사람 검토자는 DC 미리보기에서 항목별 선택 코멘트를 저장할 수 있다. 코멘트는 관련 글 번역과
AI 해설 사이에 별도 표찰 없이 평문으로 들어가며, 빈 값이면 원고에 흔적을 남기지 않는다.
코멘트를 바꾸면 기존 승인과 실패한 사전검사 영수증을 무효화하고 새 원고를 다시 승인한다.
