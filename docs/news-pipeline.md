...
자동 편집장 그림자 모드는 자동 게시 게이트 뒤에서 최근 24시간의 직접 링크와 번역문 핵심어를
...
결과는 `ready | merge | hold | hub_only` 네 값으로 API와 React 카드에만 표시한다. 아직 승인 API나
DC 게시자를 호출하지 않으며, 실제 데이터에서 오탐과 누락을 관찰한 뒤 별도 단계로 연결한다.
